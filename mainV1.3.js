...
$(window).scroll(function () {
    ...
    let tabsOffset = $('.tabs').offset().top,
        ...
        distance2 = $('.tabs-switchers').offset().top - viewport.top;
    ...
    if (distance2 <= 0 && !chekoutFormIsVisible && distance > -$('.tabs').innerHeight()) {
        if (!$('.switchers-container').hasClass('fixed')) {
            // $('.active-tab-highlight').css('opacity', '0');
            $('.switchers-container').addClass('fixed')
            // setTimeout( function () {
            //     changeHighlight();
            //     $('.active-tab-highlight').css('opacity', '1');
            // }, 300)
        }
    } else if (!tabsAreVisible || distance <= -$('.tabs').innerHeight()) {
        if ($('.switchers-container').hasClass('fixed')) {
            $('.active-tab-highlight').css('opacity', '0');
...
//Сбросить данные выбранные пользователем
...
//Сброс данных формы после её отправки
$('.close-success-btn').click(function () {
    $('input').val('')
    $('textarea').val('')
    $('input[type="submit"]').val('Send request')
    resetEstimData('Transform', transformPrice)
    $('.cartplan').eq(1).css('background', '#FFDCF5');
    $('.request-items-list.main').css('background', '#FFDCF5');
    $('.success-message .contacts-2').css('background', '#FFDCF5');
    $('#wf-form-Estimation-Form').css('display', 'flex');
})
