...
function removeOptionFromTab(clickedElement) {
    ...
    for (let i = 0; i < $('.tab-item:visible').length; i++) {
        let currentTabTtem = $('.tab-item:visible').eq(i);
        if (currentTabTtem.text() == name && currentTabTtem.attr('id') == 'clonedItem') {
            currentTabTtem.remove()
        } else if (currentTabTtem.text() == name && currentTabTtem.attr('id') !== 'clonedItem') {
            currentTabTtem.css({
                'opacity': '0',
                'display': 'none'
            });
